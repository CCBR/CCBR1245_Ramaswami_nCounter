...
ks_alone.plot = subset(ks_alone_ruv,rownames(ks_alone_ruv) %in% c(degs.both,rownames(ks_alone_nsolver_degs),
                                                       rownames(ks_alone_ruv_degs)))

ks_alone.nsolver.plot = subset(ks_alone_nsolver,
                              rownames(ks_alone_ruv) %in% 
                                c(degs.both,rownames(ks_alone_nsolver_degs),rownames(ks_alone_ruv_degs)))

# Matrix of fold change per method
ks_alone.plot.tot = data.frame(Gene = rownames(ks_alone.plot),
                              nSolverFC = ks_alone.nsolver.plot$log2FoldChange,
                              nSolverSE = ks_alone.nsolver.plot$lfcSE,
                              RUVFC = ks_alone.plot$log2FoldChange,
                              RUVSE = ks_alone.plot$lfcSE)

# Label fold change matrix
ks_alone.plot.tot$Method = ifelse(ks_alone.plot.tot$Gene %in% degs.both,'Both',
                                 ifelse(ks_alone.plot.tot$Gene %in% rownames(ks_alone_nsolver),
                                        'nSolver only','RUVSeq only'))
ks_alone.plot.tot$Contrast = 'ks_alone'

# Create the comparison dot plot data
ks_alone.plot.tot$Contrast = as.factor(ks_alone.plot.tot$Contrast)
ks_alone.plot.tot$Method = factor(ks_alone.plot.tot$Method,
                             levels = c('Both','nSolver only','RUVSeq only'))
ks_alone.plot.tot = ks_alone.plot.tot[order(ks_alone.plot.tot$Method,decreasing = T),]
...
fc.plot = ggplot(data = ks_alone.plot.tot,
